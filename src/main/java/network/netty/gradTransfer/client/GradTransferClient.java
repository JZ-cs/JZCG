...
public class GradTransferClient {
    ...
    public GradTransferClient(String selfIp, String host, int port, GradPackage gradPackage) {
        System.out.println(selfIp + "--" + "GradTransfer client init!");
        this.selfIp = selfIp;
        this.host = host;
        this.port = port;
            ...
            if (bootstrap != null) {
                ...
                f = bootstrap.connect().addListener((ChannelFuture futureListener)->{
                    ...
                    if (!futureListener.isSuccess()) {
                        System.out.printf("GradTransfer Client-%s FAILED to connected to GradTransfer Server-%s:%d! will retry in %d ms! %n", selfIp, host, port, serverNonReadyRetryIntervalMs);
                        eventLoop.schedule(() -> doConnect(new Bootstrap(), eventLoop), serverNonReadyRetryIntervalMs, TimeUnit.MILLISECONDS);
                    }else {
                        System.out.printf("GradTransfer Client-%s successfully connected to GradTransfer Server-%s:%d! %n", selfIp, host, port);
    ...
    public void run() throws InterruptedException {
        ...
        System.out.printf("GradTransfer Client-%s Closed! %n", this.selfIp);
    }
}
